...
void carica_str(char str[MAX_VET],int n)
{
	int i;
	
	for(i=0;i<n;i++) {
		printf("Inserisci una frase lettera per lettera:\n");
		scanf("%s", str[i]);
	}	
}
...
int cerca(char str[MAX_VET],int n,char el[MAX_STR])    //ricerca dicotomica
{
	int i,j,med,trovato;
...
int main () {
	int n,ris;
	printf("Quante lettere vuoi inserire (compresi li spazi) vuoi inserire?\n");
	scanf("%d",&n);
